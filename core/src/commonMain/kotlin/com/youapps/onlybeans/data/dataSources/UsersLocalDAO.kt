...
internal class UsersLocalDAO(
    ...
    suspend fun getCurrentUserData(): OBUserProfile? = withContext(Dispatchers.IO) {
        ...
        return@withContext userEmail?.run {
            onlyBeansDatabase.onlyBeansDatabaseQueries.selectCurrentUserProfile(email = userEmail)
                .executeAsOneOrNull()?.let { result ->
    ...
    suspend fun getCoffeeSpace(typeID: String): OBHomeCoffeeBar? = withContext(Dispatchers.IO) {
        when (typeID) {
            OBHomeCoffeeBarID -> {
                val coffeeBarDTO = onlyBeansDatabase.onlyBeansDatabaseQueries.selectHomeCoffeeBar()
                    .executeAsOneOrNull()
                coffeeBarDTO?.run {
                    ...
                    OBHomeCoffeeBar(
                        ...
                        coffeeGear = coffeeBarDTO.coffeeGear.runCatching {
                            Json.decodeFromString<List<OBProductListItemDTO>>(coffeeBarDTO.coffeeGear)
                                .map { itemDTO -> itemDTO.toDomain() }
                        }.getOrNull() ?: emptyList(),
                        coffeeBeans = coffeeBarDTO.coffeeBeans.runCatching {
                            Json.decodeFromString<List<OBProductListItemDTO>>(this)
                                .map { itemDTO -> itemDTO.toDomain() }
                        }.getOrNull() ?: emptyList(),
                        gallery = gallery,
