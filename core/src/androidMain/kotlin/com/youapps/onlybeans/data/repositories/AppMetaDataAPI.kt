...
internal actual class AppMetaDataAPIImpl(
    ...
    actual override suspend fun initAppData() {
        withContext(Dispatchers.IO) {
            val countryCodes: List<OBCountry> =
                applicationContext.resources.openRawResource(R.raw.countries_meta_data)
                    .bufferedReader().useLines { lines ->
                        return@useLines lines.drop(1).toList()
                    }.map {
                        val row = it.split(",")
                        OBCountry(
                            countryFlag = applicationContext.getString(
    ...
    actual override suspend fun getCountryByCode(countryCode: String): OBCountry? =
        dataSource.getCountryByCode(countryCode = countryCode)
